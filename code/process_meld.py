...
def create_window_view(df:pd.DataFrame, window:int = 5) -> List[pd.DataFrame]:
    ...
    for _, group in groups:
        for i, (_, target_row) in enumerate(group.sort_values("Utterance_ID").iterrows()):
...
def generate_emotion_prediction(dialogs:List[pd.DataFrame], emotional_labels: str) -> List[dict]:
    inputs = []
    for dialog in dialogs:
        if len(dialog) > 1:
...
def generate_speaker_ident_task(dialogs:List[pd.DataFrame], speaker_labels:str) -> List[dict]:
    inputs = []
    for dialog in dialogs:
        dialog_chain = " \t ".join(dialog["prompt"].iloc[:-1])        
        target = dialog.iloc[-1]
...
def process_dataset(dataset, window=20, speaker_task="True", predictions="True", autoregressive_emotion="True"):
    if dataset == "meld":
    ...
    for stage, ds in (("train", ds_train), ("valid", ds_dev), ("test", ds_test)):
        ...
        if speaker_task == "True":
            ...
            speaker_task_data = generate_speaker_ident_task(windowed_dialogs, speaker_labels)
            path = save_dataset(speaker_task_data, dataset, stage, "speaker")
        else:
            emotional_labels = ", ".join(ds["Emotion"].unique())
...
parser.add_argument(
    "--dataset", type=str, default="meld", help="Dataset name or path"
)
parser.add_argument(
