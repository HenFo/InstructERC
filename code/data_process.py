...
def process_dataset(dataset, window=110, speaker_task="True", predictions="True", autoregressive_emotion="True"):
    """
    ...
    emotional_dict = {
        text_label: num_label for num_label, text_label in enumerate(label_set[dataset])
    }
    speaker_label_dict = {}
    content_target_dict = {}
    speaker_target_dict = {}
    ...
    for conv_id in all_conv_id:
        for conv_turn in range(len(sentence_dict[conv_id])):
            ...
            if autoregressive_emotion == "True":
                temp_content_str += "An estimate of the emotional state for each conversation turn is provided between <> at the end of each sentence. "
            temp_content_str += "### "
            ...
            for i, (speaker_label, sub_sent) in enumerate(zip(
                ...
                # do not include last estimate of emotional state for autoregressive emotional task
                if autoregressive_emotion == "True" and index_w + i < conv_turn:
                    sub_emotion = label_set[dataset][data[1][conv_id][index_w + i]]
                    sub_sent += f" <{sub_emotion}>"
    ...
    for conv_id in all_conv_id:
        for conv_turn in range(len(data[2][conv_id])):
            ...
            speaker_target_dict[
                f"Speaker_{conv_id}_{conv_turn}"
    ...
    if predictions == "False":
                ...
                for train_id in new_train_id:
                    if train_id in content_task_dict:
                        f_train.write(
                            json.dumps(
                                {
                                    "input": f"{content_task_dict[train_id]}",
                                    "target": f"{content_target_dict[train_id]}",
                                },
                                ensure_ascii=False,
                            )
                            + "\n"
                        ...
                        f_train.write(
                            json.dumps(
                                {
                                    "input": f"{speaker_task_dict[train_id]}",
                                    "target": f"{speaker_target_dict[train_id]}",
                                },
                                ensure_ascii=False,
                            )
                            + "\n"
                ...
                for test_id in new_test_id:
                    if test_id in content_task_dict:
                        f_test.write(
                            json.dumps(
                                {
                                    "input": f"{content_task_dict[test_id]}",
                                    "target": f"{content_target_dict[test_id]}",
                                },
                                ensure_ascii=False,
                            )
                            + "\n"
                ...
                for valid_id in new_valid_id:
                    if valid_id in content_task_dict:
                        f_valid.write(
                            json.dumps(
                                {
                                    "input": f"{content_task_dict[valid_id]}",
                                    "target": f"{content_target_dict[valid_id]}",
                                },
                                ensure_ascii=False,
                            )
                            + "\n"
                        ...
                        f_valid.write(
                            json.dumps(
                                {
                                    "input": f"{speaker_task_dict[valid_id]}",
                                    "target": f"{speaker_target_dict[valid_id]}",
                                },
                                ensure_ascii=False,
                            )
                            + "\n"
                ...
                for test_id in new_test_id:
                    if test_id in speaker_task_dict:
                        f_test.write(
                            json.dumps(
                                {
                                    "input": f"{speaker_task_dict[test_id]}",
                                    "target": f"{speaker_target_dict[test_id]}",
                                },
                                ensure_ascii=False,
                            )
                            + "\n"
        ...
        with open(f"{data_path}/valid.json", "w") as f_valid:
            for valid_id in new_valid_id:
                content_task_dict[valid_id] = content_task_dict[valid_id].split("***")[
                    0
                ]
                f_valid.write(
                    json.dumps(
...
parser.add_argument(
    "--domain_base",
    type=str,
    default="select demonstration from domain_base to add to input",
    help="domain_base mode",
)
parser.add_argument(
    "--autoregressive_emotion", type=str, default="False", help="True, to include the emotional history in the input"
)
args = parser.parse_args()

# args = argparse.Namespace(dataset='meld', historical_window=20, speaker_task='True', emotion_prediction='False', domain_base='None', autoregressive_emotion='True')
